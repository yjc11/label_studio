...
def long_text_crop_and_recog(label_path, output_path, max_workers=10):
    ...
    def process_task(task):
        ...
        cur_urls = [_['page'] for _ in page_info]

        new_result_list = list()
        for i, label in enumerate(task['annotations'][0]['result']):
            if label['type'] == 'textarea':
                continue

            elif label['type'] == 'labels':
                url_idx = int(label['to_name'].split('_')[1])
                image_url = cur_urls[url_idx]
                ...
                ori_w, ori_h = label['original_width'], label['original_height']
                if (ori_h != img.shape[0]) or (ori_w != img.shape[1]):
                ...
                try:
                    ...
                    recog_res.append(
                        grpc_client("crnn_dataelem", croped_img, boxes_model)
                    )
                    recog_res.append(
                        grpc_client("ctc_revive_1.2", croped_img, boxes_model)
                    )
                ...
                text_label['from_name'] = f'transcription-{url_idx}'
    ...
    oup_json_path = Path(output_path) / f'recog_{basename}.json'
    with open(oup_json_path, 'w', encoding='utf-8') as f:
        json.dump(raw_result, f, ensure_ascii=False, indent=2)
    return oup_json_path
